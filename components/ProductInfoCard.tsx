...
const ProductInfoCard: React.FC<{ product: IProduct | any }> = ({
  product,
}) => {
  ...
  const sendRating = async () => {
    try {
      ...
      const response = await setRating(_id as string, user.token, realStar)
  ...
  const addWishlist = async () => {
    if (user && user.token) {
      try {
        const response = await addToWishList(_id as string, token)
        toast.success(response.data)
      } catch (e) {
        console.log(e)
      }
  ...
  const addToCartHandler = () => {
    ...
    dispatch(addToCart(uniqueCartItems as ICart[]))
