...
import { brands, colors } from '../../../data/productData'
...
const ProductUpdate: NextPage = () => {
  ...
  useEffect(() => {
    ...
    loadCategories()
    loadProductInfo(slug as string)
  }, [slug, router.isReady])
  ...
  const loadCategorySubs = async (categoryId: string) => {
  ...
  const loadProductInfo = async (slug: string) => {
    const response = await getProductInfo(slug)

    let preSubs: string[] = []
    response?.data?.subs?.map((sub: { _id: string }) => preSubs.push(sub._id))
    setValues((prevState) => ({
      ...prevState,
      ...response.data,
      subs: preSubs,
  ...
  const productUpdate = async (e: React.FormEvent) => {
    e.preventDefault()
    setUpdating(true)
    try {
      const response = await updateProduct(values, slug as string, authToken)
